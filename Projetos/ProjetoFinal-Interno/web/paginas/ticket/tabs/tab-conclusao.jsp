<%-- 
    Document   : tab-conclusao
    Created on : 30/08/2018, 09:25:35
    Author     : Michelle de Jesus Rogério
--%>

<!--TODO:
    - Data de Abertura (select from ticket)
    - Data de Conclusão (if (select dataconclusao from ticket is null then data conclusao
        is "Este chamado ainda não foi concluído")
    - Concluído por: +user
    - Procedimento Resolução

-->
<div class="tab-pane" id="tab-conclusao" role="tabpanel">
    Metus subtilius texit consilio fugiendam, opinionum levius amici inertissimae pecuniae
    tribus ordiamur, alienus artes solitudo, minime praesidia
    proficiscuntur reiciat detracta involuta veterum. Rutilius
    quis honestatis hominum, quisquis percussit sibi explicari.
</div>